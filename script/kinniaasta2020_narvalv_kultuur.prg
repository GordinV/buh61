...
*SET STEP ON 
gnHandle2009 = SQLConnect('narvalvpg')
gnHandle2008 = SQLConnect('narvalv2019')
If gnHandle2009 < 1
...
* asutuse nimekirje
lcString = "select id, parentid, nimetus from rekv where parentid = 119 or id = 119" 
*!*	and "+;
...
lcKpv = 'date(2020,01,01)'
lcKpv2 = 'date(2019,12,31)'
select qryRekv
scan
...
*		select * from qrySaldoKreedit1 into cursor qrySaldoKreedit
	...
	* kustutame vana lausend
	...
	lcString = "update aasta set kinni = 0 where rekvid = " + str(qryrekv.id,9)+ " and kuu = 12 and aasta = 2019"
	...
	lcString = "delete from journal where rekvid = " + str(qryrekv.id,9)+ " and kpv = date(2019,12,31) and selg ilike 'Alg.saldo%'"
	lError = sqlexec(gnhandle2009,lcString)
	if lError < 0
		messagebox('Viga.'+lcString)
		set step on
...
*!*				lcRahavoo+"','"+qrySaldoKreedit.konto+"','"+lcTp+"','999999','"+lcTp+"')"
	...
	* kustutan 
	...
	TEXT TO lcString NOSHOW textmerge
		DELETE FROM journal1 WHERE parentid in (select id FROM journal WHERE rekvid = <<qryrekv.id>> and kpv = '<<2019-12-31>>'::date)  
		and (
			LTRIM(RTRIM(deebet)) in ('103560','103930','103931','103932','103799','202000','202010','202090','203010','203020','203030','203035','206090','10010005')
		or LTRIM(RTRIM(kreedit)) in ('103560','103930','103931','103932','103799','202000','202010','202090','203010','203020','203030','203035','206090','10010005')	
		)
	endtext
