-- Function: sp_eelarve_aruanne4(integer, date, character varying, character varying, character varying, integer, integer, integer)

-- DROP FUNCTION sp_eelarve_aruanne4(integer, date, character varying, character varying, character varying, integer, integer, integer);
	...
	raise notice 'start 3500';
		select sum(summa) into lnSumma2 from ( 
			select summa from journal inner join journal1 on journal.id = journal1.parentid
				...
				and (left(journal1.deebet,3) = '100' or left(journal1.deebet,6) = '999999' ) 
				and (left(journal1.kood5 ,4) = '3500') 
			...
			select -1 * summa from journal inner join journal1 on journal.id = journal1.parentid
				where (journal.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, tnRekvId) > lnTase) or  journal.rekvid = tnRekvId)
				and (left(journal1.deebet,6) = '710000' ) 
				and (left(journal1.kood5 ,4) = '3500') 
				and year(kpv) = year(kpv) and journal.kpv <= tdKpv 
...
D 100 K 20363,20362 art 2585+D 100 K 208,2058 art 2585
*/
	select sum(summa1) into lnSumma1 from tmp_eelproj_aruanne1 where timestamp = lcreturn
	and kood5 like '2585%';	
/*
2585=(K208, K258 ) RV 05+D 100,999 art 2585



	select * 
		from curJournal where (
			(left(kreedit,3) in ('208','258') and kood3 = '05' and left(deebet,3) in ('100') and kood5 = '2585') 
			or (left(deebet,3) in ('100','999') and left(kreedit,5) in ('20363','20362') and kood5 = '2585')
			)
		and kpv <= date(2015,07,31)
		and (curJournal.rekvid in (63));
...
*/
	...
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		...
		raise notice '2585 lnsumma2 %', lnsumma2;

	-- arvestan 2586
--2586=(K208, K258 ) RV 06+K 100,999 art 2586
/*
=-1*(D 20363,20362 art 2585 K100 +D 208,2058 art 2585 K100)
=-1*(D 20363,20362,201 art 2586 K100 +D 208,2058 art 2586 K100)
2581=2580+2585+2586
...
*/
	...
	into lnsumma2 
		from curJournal where (
			((left(deebet,3) = '208' or left(deebet,4) = '2058') and kood5 = '2586' and left(kreedit,3) = '100')
			or (left(kreedit,3) in ('100') and (left(deebet,5) in ('20363','20362') or left(deebet,3) = '201') and kood5 = '2586')
			)
		and kpv <= tdKpv
		and (curJournal.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, tnRekvId) > lnTase) or curJournal.rekvid = tnRekvId);
...
--	raise notice '1502 lnSumma1 %, lnSumma2 %',lnSumma1, lnSumma2;
	...
	select sum(summa1), sum(summa2) into lnSumma1, lnSumma2 from tmp_eelproj_aruanne1 where timestamp = lcreturn
	and kood5 like '15%' and kood5 not in ('1501',  '1511', '1532', '1531');	
-- '1502','1512',
/*
	select sum(summa) into lnSumma2 
		from curJournal c
		...
		and (c.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, tnRekvId) > lnTase) or  c.rekvid = tnRekvId);
*/
	delete from tmp_eelproj_aruanne1 where kood5 like '15%' and kood5 not in ('1501',   '1511', '1532', '1531') and  timestamp = lcreturn;
-- '1502','1512',
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
...
--		raise notice '3500 lnSumma3  %',lnSumma3;
	...
	raise notice '352 eelarve %, kassa % ', lnSumma1, lnSumma2;
	...
	select sum(summa1), sum(summa2) into lnSumma1, lnSumma2 
		...
		where timestamp = lcreturn
	and kood5 like '382%' and kood5 not in  ('38250', '38251', '38252', '38254');	
...
/*
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		select '', kood5, library.nimetus, sum(summa1) as summaK1, sum(summa2) as SummaK2, lcreturn,date(),tnrekvId,'022'
			from tmp_eelproj_aruanne1
			inner join library on library.kood = tmp_eelproj_aruanne1.kood5 and library.library = 'TULUDEALLIKAD'
			where timestamp = lcReturn 
			and kood5 in ('38250', '38251', '38252', '38254') 
...
*/
	delete from tmp_eelproj_aruanne1 where timestamp = lcreturn and kood5 like '382%' and kood5 not in ('38250', '38251', '38252', '38254');
...
-- saldo seisuga tdKpv
	...
	select sum(summa1) into lnSumma3 from 	tmp_eelproj_aruanne1 where timestamp = lcReturn and kood5 like '1001%';	

	delete from tmp_eelproj_aruanne1 where timestamp = lcReturn and kood5 like '1001%';

	
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		values( '','100','Saldo seisuga'+tdkpv::text, ifnull(lnSumma3,0), ifnull(lnSumma2,0) - ifnull(lnSumma1,0),lcreturn,date(),tnrekvId,'001100'); 
...
--	delete from tmp_eelproj_aruanne1 where timestamp = lcreturn and kood5 == '100';
	...
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
		values( '','1001','Likviidsete varade muutus (+ suurenemine, - vahenemine)Saldo seisuga '+tdkpv::text,ifnull(lnsummaK1,0), ifnull(lnsumma2,0),lcreturn,date(),tnrekvId,'0011001'); 
...
-- 2580
/*
K 208,258,20362,20363
*/
	select sum(summa) into lnSumma1 from curJournal
	where kpv < date(year(tdKpv),01,01)
	and (kreedit like '208%' or kreedit like '258%' or kreedit like '20362%' or (kreedit like '20363%' and kood5 = '2580')) 
	and (curJournal.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, tnRekvId) > lnTase) or  curJournal.rekvid = tnRekvId);
...
*/
		select sum(summa1),sum(summa2) INTO lnSumma1, lnSumma2 from tmp_eelproj_aruanne1
				where  timestamp = lcreturn
				and ltrim(rtrim(kood5)) IN ('3880','3818','382','3888','3823');
...
--kokku kõik art 15
	...
	select sum(summa1), sum(summa2) into lnSumma1, lnSumma2 from tmp_eelproj_aruanne1
	where timestamp = lcReturn 
	and kood5 in ('1550','1551','1554','1555','1556','1557','156','157','158','1553','1559', '154');
...
--	raise notice 'kustutan 15 = %',lnsumma2;
	...
	delete from tmp_eelproj_aruanne1 where timestamp = lcreturn and kood5 in ('1550','1551','1554','1555','1556','1557','156','157','158','1553','1559','154');
...
--Kohustuste võtmine (+)
	...
	into lnsumma2 
		from curJournal where (
			(left(kreedit,3) in ('208','258') and kood3 = '05' and left(deebet,3) in ('100') and kood5 = '2585') 
			or (left(deebet,3) in ('100','999') and left(kreedit,5) in ('20363','20362','70001') and kood5 = '2585')
			)
		and kpv <= tdKpv
		and (curJournal.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, tnRekvId) > lnTase) or curJournal.rekvid = tnRekvId);

/*
2585 lisada +D100 K700010art 2585
*/
	raise notice '2585 lnsumma1 %, lnsumma2 %',lnsumma1, lnsumma2;
...
/*
	select sum(summa) into lnsumma3 
	from curJournal 
		where kreedit like '100%' 
		and kood5 = '2586'
		and deebet like '710010%'			
		and kpv <= tdKpv
		and (curJournal.rekvid in (SELECT id FROM rekv where fnc_get_asutuse_staatus(rekv.id, tnRekvId) > lnTase) or curJournal.rekvid = tnRekvId);
*/
	insert into tmp_eelproj_aruanne1 (kood1, kood5, nimetus, summa1, summa2, timestamp, kpv, rekvid, tunnus)
...
--	delete from tmp_eelproj_aruanne1 where kood5 in ('1554','352');
	...
	delete from tmp_eelproj_aruanne1 where kood5 in ('1554');
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION sp_eelarve_aruanne4(integer, date, character varying, character varying, character varying, integer, integer, integer) OWNER TO vlad;
GRANT EXECUTE ON FUNCTION sp_eelarve_aruanne4(integer, date, character varying, character varying, character varying, integer, integer, integer) TO public;
GRANT EXECUTE ON FUNCTION sp_eelarve_aruanne4(integer, date, character varying, character varying, character varying, integer, integer, integer) TO vlad;
GRANT EXECUTE ON FUNCTION sp_eelarve_aruanne4(integer, date, character varying, character varying, character varying, integer, integer, integer) TO dbkasutaja;


select sp_eelarve_aruanne4(63, DATE(2017,12,31),'%','%','352%','1',1,'1');



--select * from tmp_eelproj_aruanne1 where timestamp = '201801293881533KOKKU' and kood5 like '352%'
