    ...
    FROM (
             ...
             FROM (
                      ...
                      FROM journal
                               ...
                               JOIN kassatulud
                                    ON ltrim(rtrim(journal1.kreedit::TEXT)) ~~ ltrim(rtrim(kassatulud.kood::TEXT))
                               JOIN kassakontod
                                    ON ltrim(rtrim(journal1.deebet::TEXT)) ~~ ltrim(rtrim(kassakontod.kood::TEXT))
                      WHERE journal.rekvid = (CASE
                                                  WHEN $3 = 1
                                                      THEN journal.rekvid
                                                  ELSE $2 END)
             ...
             FROM (
                      ...
                      FROM journal
                               JOIN journal1 ON journal1.parentid = journal.id
                               JOIN kassakulud
                                    ON ltrim(rtrim(journal1.deebet::TEXT)) ~~ ltrim(rtrim(kassakulud.kood::TEXT))
                               JOIN kassakontod
                                    ON ltrim(rtrim(journal1.kreedit::TEXT)) ~~ ltrim(rtrim(kassakontod.kood::TEXT))
                      WHERE journal.kpv <= $1
                        AND YEAR(journal.kpv) = YEAR($1)
             ...
             FROM (
                      ...
                      WHERE journal.kpv <= $1
                        ...
                        AND NOT empty(journal1.kood5)
                        AND journal1.kood5 IN
                            (SELECT kood FROM library WHERE library.library = 'TULUDEALLIKAD' AND tun5 = 2)
                      GROUP BY journal.rekvid, journal1.kood1, journal1.kood2, journal1.kood5
                  ) kassatulu
         ) qry
